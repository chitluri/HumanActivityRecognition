...
#Clean 561 attributes in X_test
cat test/X_test.txt | awk '{out=$1; for(i=2;i<=NF;i++){out=out","$i}; print out}' > test/X_test_clean.txt
...
cat test/header.arff test/data.csv > test/testingData.arff
