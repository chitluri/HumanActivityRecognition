...
cat train/X_train.txt | awk '{out=$1; for(i=2;i<=NF;i++){out=out" "$i}; print out}' > train/X_train_clean.txt
...
cat train/header.arff train/data.csv > train/trainingData.arff
